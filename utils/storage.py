...
class Storage:
    ...
    def add_contact(self, name: str, phone: str, relation: str = "Family"):
        ...
        contact = {"id": len(contacts) + 1, "name": name, "phone": phone, "relation": relation, "added": datetime.now().isoformat()}
    ...
    def add_history_event(self, event_type: str, location: dict = None, cancelled: bool = False):
        history = self._read(self.history_file) or []
        event = {"id": len(history) + 1, "type": event_type, "timestamp": datetime.now().isoformat(), "location": location or {}, "cancelled": cancelled}
